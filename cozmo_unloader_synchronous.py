...
def custom_objects(robot: cozmo.robot.Robot):
    ...
    targetLocation1= robot.world.define_custom_cube(CustomObjectTypes.CustomType00,
                                                CustomObjectMarkers.Hexagons3,
                                                5,50,50,True)

    targetLocation2= robot.world.define_custom_cube(CustomObjectTypes.CustomType01,
                                                CustomObjectMarkers.Triangles2,
                                                5,
                                                50,50,True)

    targetLocation3= robot.world.define_custom_cube(CustomObjectTypes.CustomType02,
                                                CustomObjectMarkers.Circles2,
                                                5,
                                                50,50,True)
    ramp= robot.world.define_custom_cube(CustomObjectTypes.CustomType03,
                                                CustomObjectMarkers.Diamonds2,
                                                1,
                                                50,50,True)
    topBack= robot.world.define_custom_cube(CustomObjectTypes.CustomType04,
                                                CustomObjectMarkers.Diamonds4,
                                                1,
                                                50,50,True)
    frontOfBox= robot.world.define_custom_cube(CustomObjectTypes.CustomType05,
                                                CustomObjectMarkers.Hexagons2,
                                                1,
                                                50,50,True)
    topSideWall= robot.world.define_custom_cube(CustomObjectTypes.CustomType06,
    ...
    if ((targetLocation1 is not None) and (targetLocation2 is not None) and
            (targetLocation3 is not None) and (ramp is not None)):
                ...
                if(obj.cube_id):
                    ...
                    if(self.allLocFound):
                        ...
                        if(not self._checkDistToLoc(num,obj)):
                            print('cube '+ str(obj.object_id)+ 'has already been delivered')
                            return
                        ...
                        if(self.action.has_succeeded):
                            ...
                            if(num==1):
                                self.action=self._cozmo.go_to_pose(self.loc1Pose)
                                self.action.wait_for_completed()
                                if(self.action.has_succeeded):
